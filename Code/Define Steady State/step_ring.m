function [nextX, nextY, currentState] = step_ring(robot, dimension, dt)
%STEP_RING Move robots according to control law and boundary law.
...
%	@param dt		scalar		is the step time
    ...
    if currentState == 0
        ...
        F = mapFunction_ring(robot.x, robot.y, dimension); % map is the scalar field
        D = 1/sqrt(F);  % let c=1
        dX = D*randn() *sqrt(dt);
        dY = D*randn() *sqrt(dt);
