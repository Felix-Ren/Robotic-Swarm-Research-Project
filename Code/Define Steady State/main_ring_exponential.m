...
for i=1:nSteps
    ...
    for j = 1:nRobots
        [robots(j).x, robots(j).y, robots(j).state] =...
            step_ring(robots(j), dimension, dt); 
    end
...
%     subplot('position', [0.48,0.4,0.45,0.55]);
    score(i) = coverageTest_ring(robots, dimension, 0.2021); 
...
%          dt:dt:duration, ones(1,size(dt:dt:duration,2)).*lowErrBd, ':black', ...
%          dt:dt:duration, ones(1,size(dt:dt:duration,2)).*uppErrBd, ':red');
end

%% fit exponential decay to the error curve
xAxis = double(dt:dt:duration);

% set the model: a*exp(b*x)+c
expModelOpt = fitoptions('exp2');
expModelOpt.lower = [-Inf -Inf -Inf 0];
expModelOpt.upper = [Inf Inf Inf 0];
expFit = fit(xAxis', score, 'exp2', expModelOpt);
...
plot(xAxis, score, 'blue', ...                                % error
     xAxis, fittedVal, 'red', ...                             % fitted value
     xAxis, ones(1,size(xAxis,2))*expModelThresh, '--green')  % draw exp model threshold line
axis tight
ttl = ['Error Metric v.s. Time, Ring Pattern'];
title(ttl, 'interpreter', 'latex')
leg = {'error metric','fitted exponential curve','98\% drop threshold'};
legend(leg, 'interpreter', 'latex', 'location','best')
ylabel('error metric $e^{\delta = 2in}_{N=200}(t)$','interpreter', 'latex')
